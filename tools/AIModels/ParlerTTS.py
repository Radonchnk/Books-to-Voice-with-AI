...
class ParlerTTSModel:
    ...
    def textToMP3(self, text, outputFile):
        text = self.replaceNumbersWithWords(text)
        text = unidecode(text)
        waveformOutput = self.textToSpeech(text)

        self.saveToFile(waveformOutput, outputFile)
...
if __name__ == "__main__":
    tts = ParlerTTSModel("parler-tts/parler-tts-mini-expresso", "cuda",
